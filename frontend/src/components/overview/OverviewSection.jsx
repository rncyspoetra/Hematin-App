  ...
  const handleSubmit = async () => {
    try {
      ...
      const res = await axios.post(
        "http://localhost:2000/predictions", answers
      );
      ...
      <div className="max-w-3xl w-full bg-white rounded-xl shadow p-6 space-y-8">
              ...
              {seg.input.map((inp, j) => (
                <div key={j}>
                  ...
                  <p className="text-base text-gray-500 font-medium">
                    {inp.satuan == "Rp. " ? inp.satuan : ""} {answers[inp.name] || "-"} {inp.satuan == "Tahun" ? inp.satuan : ""}
                  </p>
                </div>
              ))}
            </div>
        ...
        <div className="flex justify-between mt-10">
          ...
          <button
            ...
            className="font-semibold flex items-center gap-2 px-4 py-2 bg-[#f9f9f9] text-[#5AA9E6] rounded-md border-2 border-[#5AA9E6] hover:bg-[#5AA9E6] hover:border-transparent hover:text-[#f9f9f9] focus:outline-none"
          >
            Submit
          </button>
        </div>
